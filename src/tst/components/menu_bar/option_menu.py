...
class OptionMenu(tk.Menu):
    ...
    def _handle_open_option(self, event: tk.Event = None) -> None:
        ...
        self.textbox_token.insert(0, self._config['CLIENT']['CLIENT_ACCESS_TOKEN'])
        ...
        self.textbox_num.insert(0, self._config['DEFAULT']['Search'])
        self.textbox_num.grid(row=1, column=1, padx=5, pady=5)

        ttk.Button(self.option_window, text='保存', command=self._handle_save).grid(row=2, column=0, pady=5)
    ...
    def _handle_save(self, event: tk.Event = None) -> None:
        self._config['CLIENT']['CLIENT_ACCESS_TOKEN'] = self.textbox_token.get()
        self._config['DEFAULT']['Search'] = self.textbox_num.get()
        with open(self.CONFIG_PATH, 'w') as configfile:
            self._config.write(configfile)
